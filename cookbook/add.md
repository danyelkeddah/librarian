...
Register a new skill, agent, or prompt in the librarian catalog.
...
- Verify each dependency already exists in `librarian.yaml` or warn the user
  - If they don't exist add them to `librarian.yaml` first. If those files have dependencies, add them recursively.
...
### 5. Add the Entry to librarian.yaml

Read `librarian.yaml`, add the new entry under the correct section:
...
git commit -m "librarian: added <type> <name>"
git push
```

### 7. Confirm

Tell the user the entry has been added and is now available for others to use via `/librarian use <name>`.
