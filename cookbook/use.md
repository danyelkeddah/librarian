...
- If no match, tell the user and suggest `/librarian search`
...
- Read `default_dirs` from `librarian.yaml`
...
- For skills: copy the entire parent directory to the target:
  ```bash
  cp -R <parent_directory>/ <target_directory>/<n>/
  ```
...
- Copy the parent directory of the file to the target:
  ```bash
  cp -R "$tmp_dir/<parent_path>/" <target_directory>/<n>/
  ```
- Clean up:
  ```bash
  rm -rf "$tmp_dir"
